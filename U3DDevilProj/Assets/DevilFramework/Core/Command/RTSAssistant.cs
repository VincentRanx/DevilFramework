    ...
    public class RTSAssistant
    {
        ...
        public bool Assistant(InputReader ireader, IRTSEngine engine)
        {
            ...
            if (!string.IsNullOrEmpty(keyword))
            {
                ...
                KeyWord = keyword;
                bool f;
                Dictionary<string, IRTSFunction> funcs = Ref.GetField(engine, "mFuncs", out f) as Dictionary<string, IRTSFunction>;
                int flen = funcs == null ? 0 : funcs.Count;
                Dictionary<string, object> vars = Ref.GetField(engine, "mVars", out f) as Dictionary<string, object>;
