Shader "DevilTeam/BlitImg"
{
	Properties
	{
		...
		_Color ("Simple Color", Color) = (1,1,1,1)
		_ClipRect ("Clip Rect (min.xy, max.xy)", Vector) = (0,0,1,1)
	}
...
#define WEIGHT float3(0.147761, 0.118318, 0.0947416)
	...
	v2f vert_blur(appdata_img v)
	{
		...
		//uv坐标  
		o.uv = lerp(_ClipRect.xy, _ClipRect.zw, v.texcoord.xy);

		half2 offset = _MainTex_TexelSize.xy * _BlurStep * _BlurIters;
		o.uvOffset = offset * (_ClipRect.zw - _ClipRect.xy);
