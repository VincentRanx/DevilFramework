...
namespace Devil.Utility
{
    ...
    {
#if UNITY_EDITOR
        static bool MatchBindName(string targetName, string refName, bool isArray)
        {
            ...
            {
                string pattern = @"^name( \([0-9]+\)|_[0-9]+)?$";
                pattern = pattern.Replace("name", refName);
        ...
        public static void BindPropertyiesByName(this MonoBehaviour mono)
        {
            ...
            for (int i = 0; i < fields.Length; i++)
            {
                ...
                for (int j = children.Count - 1; j >= 0; j--)
                {
                    ...
                    if (MatchBindName(trans.name, fld.Name,isArray))
                    {
                        children.RemoveAt(j);
                        if (isGameObj)
                        {
                ...
                if (isArray)
                {
                    GlobalUtil.Sort(targets, (x, y) => x.name.CompareTo(y.name));
