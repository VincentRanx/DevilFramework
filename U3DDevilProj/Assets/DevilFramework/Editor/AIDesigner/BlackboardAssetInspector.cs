...
namespace DevilEditor
{
    ...
    public class BlackboardAssetInspector : Editor
    {
        ...
        public static void OnBlackboardInspectorGUI(BlackboardAsset asset, float fixHeight = 0)
        {
            EditorGUILayout.BeginVertical("helpbox");
            EditorGUILayout.BeginHorizontal((GUIStyle)"IN Title");
            var drop0 = EditorPrefs.GetBool("BT", true);
            var drop = EditorGUILayout.Foldout(drop0, "Blackboard", true);
            EditorGUILayout.EndHorizontal();
            if (drop ^ drop0)
                EditorPrefs.SetBool("BT", drop);
            //drop = QuickGUI.DrawHeader("Blackboard", "BT", false);
            if (drop)
            {
                EditorGUI.indentLevel++;

                bool fix = fixHeight > 1;
                if (fix)
                    scroll = EditorGUILayout.BeginScrollView(scroll, GUILayout.Height(fixHeight));
                ...
                if (!Application.isPlaying)
                {
                    ...
                    dropIndex = EditorGUILayout.Popup(dropIndex, AIModules.SharedTypeNames);
                    EditorGUILayout.EndHorizontal();
                    EditorGUILayout.BeginHorizontal();
