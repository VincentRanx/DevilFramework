...
%pattern%
        ...
        List<BTSharedValue> mSharedType = new List<BTSharedValue>(50); // 可用于行为树的数据类型
        ...
        {
            mSharedType.Clear();
            var tp = new BTSharedValue(typeof(int));
            tp.Ctor = (x) =>
            {
                int num;
            ...
            mSharedType.Add(tp);
            tp = new BTSharedValue(typeof(float));
            tp.Ctor = (x) =>
            {
                float num;
                if (float.TryParse(x, out num))
                    return num;
                else
                    return 0;
            };
            mSharedType.Add(tp);
            tp = new BTSharedValue(typeof(string));
            tp.Ctor = (x) => x;
            mSharedType.Add(tp);
            tp = new BTSharedValue(typeof(bool));
            tp.Ctor = (x) => x != null ? x.ToLower() == "true" : false;
            mSharedType.Add(tp);
            tp = new BTSharedValue(typeof(Vector3));
            tp.Ctor = (x) => StringUtil.ParseVector3(x);
            mSharedType.Add(tp);
            tp = new BTSharedValue(typeof(GameObject));
            tp.Ctor = (x) => GameObject.Find(x);
            mSharedType.Add(tp);
            tp = new BTSharedValue(typeof(Transform));
            tp.Ctor = (x) =>
            {
                var o = GameObject.Find(x);
                return o == null ? null : o.transform;
            };
            mSharedType.Add(tp);
            LoadModules();
            ...
            {
                SharedTypeNames[i] = mSharedType[i].Name;
            }
        }
        ...
        void ParseAILibs(string scriptPath)
        {
            ...
            if (t == null)
                return;
            // 解析数据类型
            var stp = Ref.GetCustomAttribute<BTSharedTypeAttribute>(t);
            if(stp != null)
            {
                var svalue = new BTSharedValue(t);
                mSharedType.Add(svalue);
            }
