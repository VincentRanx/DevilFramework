...
namespace DevilEditor
{
    ...
    public class BehaviourTreeDesignerWindow : EditorCanvasWindow
    {
        ...
        public BTNodeBase BreakNode { get; set; }
        bool mLockTarget;
        ...
        public BehaviourTreeDesignerWindow() : base()
        {
            ...
            GraphCanvas.AddElement(new BehaviourHelpGUI());
        ...
        protected override void OnEnable()
        {
            ...
            AddDelayTask(TASK_RESELECT, ReloadGraph);
        }
        ...
        public void ReloadBehaviourTree()
        {
            AddDelayTask(TASK_RESELECT, ReloadGraph);
        }
        ...
        void BindBehaviourTreeEvent(bool bind)
        {
            ...
            if (bind)
            {
                ...
                Runner.OnMonoStart += OnBehaviourMonoStart;
                OnBehaviourMonoStart(Runner);
            }
            else
            {
                Runner.OnBehaviourTreeFrame -= OnBehaviourTreeFrame;
                Runner.OnMonoStart -= OnBehaviourMonoStart;
                BindRuntimeNode(false);
            }
        }

        void BindRuntimeNode(bool bind)
        {
            for (int i = 0; i < TreeCanvas.ElementCount; i++)
            {
                ...
                if (node == null)
                    continue;
                node.RuntimeNode = bind ? Runner.FindRuntimeNode(node.Self.BTId) : null;
            }
        }

        private void OnBehaviourMonoStart(BehaviourTreeRunner btree)
        {
            if(btree == Runner)
                BindRuntimeNode(true);
        }
        ...
        {
            if (IsPlaying || ContextMenu.Visible)
                return false;
            if (key == KeyCode.D)
            {
                int n = CopyNodes((x) => x.IsSelected, Event.current.control, new Vector2(50, 20));
                if (n > 0)
                    RebuildExecutionOrder();
                return true;
            }
        ...
        public int CopyNodes(FilterDelegate<BehaviourNodeGUI> selector, bool keepparent, Vector2 offset)
        {
            List<BehaviourNodeGUI> list = new List<BehaviourNodeGUI>();
            for (int i = 0; i < TreeCanvas.ElementCount; i++)
            {
                BehaviourNodeGUI node = TreeCanvas.GetElement<BehaviourNodeGUI>(i);
            ...
            List<BehaviourNodeGUI> copy = new List<BehaviourNodeGUI>();
            foreach (var node in list)
            {
                var newnode = node.Copy(offset);
                copy.Add(newnode);
                AddChild(null, newnode);
            ...
            {
                var node = list[i];
                BehaviourNodeGUI parent = TreeGraph.GetParent(0, node) as BehaviourNodeGUI;
                if (parent == null)
                    continue;
                int n = list.IndexOf(parent);
                if (n < 0 && keepparent)
                    TreeGraph.AddPath(0, parent, copy[i]);
                else if (n >= 0)
                    TreeGraph.AddPath(0, copy[n], copy[i]);
            }
            return list.Count;
        }
        ...
        public void AddChild(PaintElement parent, BehaviourNodeGUI node)
        {
            TreeCanvas.AddElement(node);
            TreeGraph.AddNode(node);
            if (parent != null)
