...
namespace DevilEditor
{
    public class BehaviourNodeGUI : PaintElement
    {
        ...
        public class Decorator
        {
            ...
            public Color CacheColor { get; set; }
            ...
            private Decorator(int id)
            {
                BTId = id;
            }

            public Decorator Copy(int id)
            {
                Decorator decor = new Decorator(id);
                decor.BTMeta = BTMeta;
                decor.PropertiesInfo = PropertiesInfo;
                decor.TextHeight = TextHeight;
                decor.SubTextHeight = SubTextHeight;
                decor.TimeOffset = TimeOffset;
                decor.NotFlag = NotFlag;
                decor.Properties = new BTInputProperty[Properties.Length];
                for (int i = 0; i < Properties.Length; i++)
                {
                    decor.Properties[i] = Properties[i].Copy();
                }
                return decor;
            }
        ...
        public BTNodeBase RuntimeNode
        {
            get { return mRuntimeNode; }
            set
            {
                if (mRuntimeNode != value)
                {
                    if (mRuntimeNode != null)
                    {
                        mRuntimeNode.StartRoundHandler = null;
                        mRuntimeNode.StopRoundHandler = null;
                    }
                    mRuntimeNode = value;
                    if (mRuntimeNode != null)
                    {
                        mRuntimeNode.StartRoundHandler = OnRuntimeNodeBegin;
                        mRuntimeNode.StopRoundHandler = OnRuntimeNodeEnd;
                    }
                }
            }
        }
        ...
        // 运行时节点开始
        void OnRuntimeNodeBegin(BTNodeBase node)
        {
            if(node == RuntimeNode && BreakToggle)
            {
                Debug.Break();
                mWindow.BreakNode = node;
            }
        }

        // 运行时节点结束
        void OnRuntimeNodeEnd(BTNodeBase node)
        {

        }

        public BehaviourNodeGUI Copy(Vector2 offset)
        {
            BehaviourNodeGUI copy = new BehaviourNodeGUI(mWindow);
            copy.Self = Self.Copy(mWindow.GenerateId);
            BTExecutionOrder = -1;
            foreach(var decor in conditions)
            {
                copy.conditions.Add(decor.Copy(mWindow.GenerateId));
            }
            foreach(var decor in services)
            {
                copy.services.Add(decor.Copy(mWindow.GenerateId));
            }
            Rect r = LocalRect;
            r.position += offset;
            copy.LocalRect = r;
        ...
        public override void OnGUI(Rect clipRect)
        {
            ...
            string style;
            if (EditorApplication.isPaused &&  RuntimeNode != null && 
                mWindow.BreakNode == this.RuntimeNode && Mathf.Repeat((float)EditorApplication.timeSinceStartup, 1) < 0.5f)
            ...
            BreakToggle = GUI.Toggle(r, BreakToggle, "", "Grad Down Swatch Overlay");
        }
        ...
        public override bool InteractMouseClick(EMouseButton button, Vector2 mousePositoin)
        {
            ...
            else if (button == EMouseButton.right && !EditorApplication.isPlayingOrWillChangePlaymode)
            {
                mWindow.BeginEditNode(null, BehaviourTreeDesignerWindow.ENodeEditMode.none);
            ...
            else if(mRaycastDecorator!= null && mRaycastDecorator.BTMeta.NodeType == EBTNodeType.condition && !EditorApplication.isPlayingOrWillChangePlaymode)
            {
