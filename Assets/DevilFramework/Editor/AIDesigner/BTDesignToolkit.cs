...
namespace DevilTeam.Editor
{
    ...
    public class BTModule
    {
        ...
        public static string ModuleTarget(string moduleId)
        {
            if (moduleId == null)
                return BTDesignToolkit.EMPTY;
            int n = moduleId.IndexOf('-');
            return n >= 0 ? moduleId.Substring(0, n) : BTDesignToolkit.EMPTY;
        }
    ...
    public class BTNode
    {
        ...
        public string text = "";
        string _moduleId = "";
        public ESortType sortMode = ESortType.none;
        ...
        public string DisplayName
        {
            get
            {
                if (!string.IsNullOrEmpty(name))
                    return name;
                if ((((1 << nodeType) & BTDesignToolkit.MODULES_NODES) != 0) && !string.IsNullOrEmpty(moduleName))
                    return moduleName;
                if (nodeType >= 0 && nodeType < BTDesignToolkit.nodeNames.Length)
                    return BTDesignToolkit.nodeNames[nodeType];
                return "";
            }
        }
        ...
        static BTNode ReadXmlElement(XmlElement root)
        {
            ...
            node.sortMode = (ESortType)int.Parse(root.GetAttribute("sort"));
        ...
        static XmlElement GetElement(XmlDocument xml, BTNode node)
        {
            ...
            root.SetAttribute("sort", ((int)node.sortMode).ToString());
    ...
    public class BTDesignToolkit
    {
        ...
        public const uint MODULES_NODES = 0
            | (1u << (int)EBTNode.behaviour)
            | (1u << (int)EBTNode.condition)
            | (1u << (int)EBTNode.patrol)
            | (1u << (int)EBTNode.custom_control);
        ...
        public static string[] nodeNames = { "空节点", "序列", "选择", "队列", "自定义", "行为", "条件", "巡逻", "AI 库", "预设" };
...
这是一个特殊节点，您可以在它下面创建一些常用的行为树作为预设，以后可以快速选择其中的内容，它的所有子节点平行，表示一个个的预设体。";
        ...
        {
            BTNode prefab;
            prefab = BTNode.ReadXml(Path.Combine(Installizer.InstallRoot, "DevilFramework/Editor/AIDesigner/BTRepository.xml"));
            if (prefab != null)
            {
        ...
        NODE CreateNode(BTNode node)
        {
            ...
            nd.UserData.m_SortType = node.sortMode;
        ...
        BTNode GetNode(NODE origin)
        {
            ...
            node.sortMode = origin.UserData.m_SortType;
        ...
        int GetNodeTypeMask(BTNode node)
        {
            uint ret = 0;
            ret |= CTRL_NODES;
            if (node.Length == 1)
        ...
        public void DrawNodeEditor(Vector2 viewOffset)
        {
            ...
            contextMenuRect.size = new Vector2(250, 200);
            contextMenuRect.position = new Vector2(editNode.position.xMax + nodeFrameSize.x, editNode.position.yMin - nodeFrameSize.y) + viewOffset;
            GUILayout.BeginArea(contextMenuRect, editNode.DisplayName, "Window");
            contextScroll = GUILayout.BeginScrollView(contextScroll);
          
            DrawTypeList(editNode);
            DrawModuleList(editNode);
            DrawNodeInfo(editNode);
        ...
        void DrawNodeInfo(BTNode node)
        {
            GUILayout.BeginVertical("button", GUILayout.MinHeight(30));

            GUILayout.BeginHorizontal();
            GUILayout.Label("命名", GUILayout.Width(40));
            editNode.name = GUILayout.TextField(editNode.name);
            GUILayout.EndHorizontal();

            GUILayout.BeginHorizontal();
            GUILayout.Label("优先级", GUILayout.Width(40));
            node.priority = EditorGUILayout.IntField(node.priority);
            GUILayout.EndHorizontal();

            if (node.Length > 1 && node.NodeType != NODE_PATROL)
            {
                GUILayout.BeginHorizontal();
                GUILayout.Label("排序", GUILayout.Width(40));
                node.sortMode = (ESortType)EditorGUILayout.EnumPopup(node.sortMode);
                GUILayout.EndHorizontal();
            }
            GUILayout.BeginHorizontal();
            GUILayout.Label("INT", GUILayout.Width(40));
            node.intInput = EditorGUILayout.IntField(node.intInput);
            GUILayout.EndHorizontal();

            GUILayout.BeginHorizontal();
            GUILayout.Label("FLOAT", GUILayout.Width(40));
            node.floatInput = EditorGUILayout.FloatField(node.floatInput);
            GUILayout.EndHorizontal();
        ...
        void DrawTypeList(BTNode node)
        {
            GUILayout.BeginHorizontal("button", GUILayout.MinHeight(30));
            GUILayout.Label("节点", GUILayout.Width(40));
            int mask = GetNodeTypeMask(node);
            if (showTypes && mask != 0)
            {
                GUILayout.BeginVertical();
                for (int i = 0; i < NODE_REPOSITORY; i++)
                {
                    if ((mask & (1 << i)) == 0)
                        continue;
                    bool sel0 = i == node.NodeType;
                    bool sel = GUILayout.Toggle(sel0, nodeNames[i], "PreDropDown");
                    if (sel ^ sel0)
                    {
                        node.NodeType = i;
                        showTypes = false;
                        if (((1 << i) & MODULES_NODES) != 0)
                        {
                            showModules = true;
                        }
                        break;
            ...
            {
                showTypes = GUILayout.Toggle(showTypes, nodeNames[node.NodeType], "PreDropDown") && mask != 0;
                if (showTypes)
                    showModules = false;
            }
        ...
        void DrawModuleList(BTNode node)
        {
            List<BTModule> modules = null;
            ...
            if (modules == null || modules.Count == 0)
                return;
            GUILayout.BeginHorizontal("button", GUILayout.MinHeight(30));
            GUILayout.Label("模块", GUILayout.Width(40));
            if (showModules)
            {
                GUILayout.BeginVertical();
                BehaviourModule mod = null;
                for (int i = 0; i < modules.Count; i++)
                {
                    if (mod != modules[i].module)
                    {
                        mod = modules[i].module;
                        GUILayout.Label(mod.GetType().Name, "EyeDropperHorizontalLine");
                    }
                    bool sel0 = modules[i].id == node.moduleId;
                    bool sel = GUILayout.Toggle(sel0, modules[i].name, "PreDropDown");
                    if (sel ^ sel0)
                    {
                        showModules = false;
                        node.moduleId = modules[i].id;
                    }
                }
                GUILayout.EndVertical();
            }
            ...
            {
                showModules = GUILayout.Toggle(showModules, node.moduleName, "PreDropDown");
                if (showModules)
                    showTypes = false;
            }
            GUILayout.EndHorizontal();
        ...
        void DrawNodeTool(BTNode node, Vector2 viewOffset)
        {
            ...
            bool b;
            if (node.Parent != null && node.NodeType < NODE_REPOSITORY && !showContextMenu)
            {
        ...
        public static uint DrawNodeTitile(BTNode node, Vector2 viewOffset)
        {
            ...
            rect.position = pos;
            GUI.Label(rect, nodeNames[node.NodeType], "ChannelStripAttenuationMarkerSquare");
            return 0;
        }

        void DrawName(BTNode node, Vector2 viewOffset)
        {
            string s = node.DisplayName;
        ...
        public void ConnectNode(BTNode from, BTNode to, Vector2 viewOffset, Color color, float width = 3)
        {
            ...
            rect.center = tmpP2;
            if (!(showContextMenu || editCommentNode != null) && rect.Contains(Event.current.mousePosition))
            {
                rect.size = radioSize;
                rect.center = tmpP2;
        ...
        public void OnPaint(BTNode node, ref Vector2 viewOffset)
        {
            ...
            if (raycastNode == node && !showContextMenu && editCommentNode == null)
            {
